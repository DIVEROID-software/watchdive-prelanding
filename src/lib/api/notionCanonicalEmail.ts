...
export async function withCanonicalEmailShape<T>(run: () => Promise<T>): Promise<T> {
  const attemptedShape = currentShape;
  try {
    return await run();
  } catch (error) {
    if (!isCanonicalEmailShapeError(error)) throw error;
    // Use the opposite of the shape this request actually attempted. Two
    // concurrent first requests can both fail with the same stale shape; a
    // blind toggle of global state would flip twice and undo the recovery.
    currentShape = attemptedShape === "email" ? "rich_text" : "email";
    return run();
  }
}
