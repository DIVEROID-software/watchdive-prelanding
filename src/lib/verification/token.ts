...
export { isVerificationTokenShape, verificationTokenLocale } from "./tokenShape.ts";
...
export type ParsedToken = {
  ...
  /** Signed UI/email locale. Legacy v1 links resolve to English. */
  locale: Locale;
};

/**
 * `v2.<leadId>.<expiry seconds>.<consent>.<locale>.<mac>`.
 *
 ...
 * something this person declined. The locale is carried alongside it so the
 * verification page and delayed welcome mail use the language chosen at
 * submit time without adding a new CRM column. Neither value is an address or
 * identifier.
 *
 * The preceding v1 shape (`<leadId>.<expiry>.<consent>.<mac>`) remains
 * parseable so links already delivered before localization keep working. They
 * deterministically resolve to English.
 */
export function createVerificationToken(
  ...
  if (!isLocale(locale)) throw new Error("Invalid token locale");
  const payload = `v2.${leadId.toLowerCase()}.${expiresAtSeconds}.${consentFlag(measurementConsent)}.${locale}`;
  return `${payload}.${sign(secret, `verify:v2:${payload}`)}`;
}
...
export function parseVerificationToken(
  ...
  const [leadId, expiresAtSeconds, consent, locale, mac] = legacy
    ? [parts[0], parts[1], parts[2], DEFAULT_LOCALE, parts[3]]
    : [parts[1], parts[2], parts[3], parts[4], parts[5]];
  if (!leadId || !expiresAtSeconds || !consent || !isLocale(locale) || !mac) return undefined;
  const payload = legacy
    ? `${leadId.toLowerCase()}.${expiresAtSeconds}.${consent}`
    : `v2.${leadId.toLowerCase()}.${expiresAtSeconds}.${consent}.${locale}`;
  // Flipping the consent or locale segment changes the signed payload, so a
  // tampered token cannot upgrade a denial or redirect the experience into a
  // different language.
  if (!safeEqual(mac, sign(secret, `verify:${legacy ? "v1" : "v2"}:${payload}`))) {
    return undefined;
  }
...
export function verificationUrl(publicOrigin: string, token: string): string {
  ...
  const locale = verificationTokenLocale(token);
  if (!locale) throw new Error("Invalid verification token locale");
  const url = new URL(verifyPath(locale), publicOrigin);
  url.hash = token;
  return url.toString();
}
